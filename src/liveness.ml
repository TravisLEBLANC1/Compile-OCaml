...
let rec use_expr e = match e with
  | Int _ | Bool _ | Addr _-> VSet.empty
  | Var x -> VSet.singleton x
  | Binop (_, e1, e2) -> VSet.union (use_expr e1) (use_expr e2)
  | Unop(_,e) | Deref e | Sbrk e -> use_expr e 
  | Call(_, le) -> List.fold_left (fun acc e -> VSet.union acc (use_expr e)) VSet.empty le
  | PCall(e, le) -> 
    let use_call = List.fold_left (fun acc e -> VSet.union acc (use_expr e)) VSet.empty le in
    VSet.union use_call (use_expr e)
...
let liveness fdef =
  ...
  let rec lv_in_instr (i:instruction) lv_out = 
    let s = match i.instr with
      ...
      | Write(e1, e2) -> VSet.union lv_out (VSet.union (use_expr e1) (use_expr e2))
    in
    live.(i.nb) <- s;
    s;
...
let liveness_intervals_from_liveness fdef =
  ...
  let update_vmap (index:int) (lv_in:VSet.t) = 
    ...
    | Some (low, _) -> vmap := VMap.add s (low, index) !vmap
